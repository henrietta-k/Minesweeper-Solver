...
class Game:
    ...
    def __init__(
        self,
        windowWidth: int = COLUMNS,
        windowHeight: int = ROWS,
    ...
    def on_init(self) -> None:
        ...
        if self.render:
            ...
            self._display_surf = pygame.display.set_mode(
                (
                    self.windowWidth * self.blockSize,
                    self.windowHeight * self.blockSize + 80,
                ),
                pygame.HWSURFACE,
            )
    ...
    def on_loop(self) -> None:
        ...
        self.loop_count += 1
        if self.verbose > 0:
            progress_percent = 1 - self.board.remaining / (
                self.windowHeight * self.windowWidth - self.board.mines
            )
            progress_bar = "▉" * int(progress_percent * 40)
            print(
                f"Loop Count {self.loop_count:3d}: Blocks remaining = {self.board.remaining:4d} [{progress_bar:40s}]"
            )
    ...
    def on_render(self) -> None:
        ...
        if self._running:
            time_text = self.font.render(
                f"{time.time() - self.time:.0f}s", False, (255, 255, 255)
            )
        else:
            time_text = self.font.render(
                f"{self.win_time - self.time:.0f}s", False, (255, 255, 255)
            )
        self._display_surf.blit(
            time_text, (80, self.windowHeight * self.blockSize + 10)
        )
        remaining_text = self.font.render(
            str(self.board.flags_remaining), False, (255, 255, 255)
        )
        self._display_surf.blit(
            remaining_text,
            (
                self.windowWidth * self.blockSize - 80,
                self.windowHeight * self.blockSize + 10,
            ),
        )
        pygame.display.flip()
    ...
    def win(self) -> None:
        ...
        if self.render:
            ...
            textRect.center = (
                self.windowWidth * self.blockSize // 2,
                self.windowHeight * self.blockSize // 2,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--columns", "-c", type=int, default=COLUMNS, help="Board columns"
    )
    parser.add_argument(
        "--mines", "-m", type=int, default=MINES, help="Number of mines on the board"
    )
    parser.add_argument(
        "--seed", "-s", help="Provide a seed to recreate the same boards."
    )
    parser.add_argument(
        "--no-render",
        action="store_true",
        help="Disable rendering for testing purposes",
    )
    parser.add_argument(
        "--block-size",
        "-b",
        type=int,
        default=BLOCK_SIZE,
        help="Size of each block in pixels",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    args = parse_args()
    game = Game(
        windowWidth=args.columns,
        windowHeight=args.rows,
