...
class MinesweeperLPSolver():
    ...
    def __init__(self, board: Board, verbose: int = 0) -> None:
        ...
        self.x: cp.Variable = cp.Variable([self.board.windowWidth, self.board.windowHeight])
        self.constraints: List[cp.constraints.constraint.Constraint] = [self.x >= 0, 1 >= self.x, cp.sum(self.x) == self.board.mines]
        self.objective: cp.problems.objective.Objective = cp.Maximize(0) #Fake objective
        self.last_solution: np.ndarray = np.empty([self.board.windowHeight, self.board.windowWidth])
