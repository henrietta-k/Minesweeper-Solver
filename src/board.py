...
class Board:
    ...
    def create(self) -> None:
        ...
        self.neighboring_mine_count = [
            [0] * self.windowWidth for _ in range(self.windowHeight)
        ]
        self.remaining = self.windowWidth * self.windowHeight - self.mines
        self.flags_remaining = self.mines
        self._bombed = False
        ...
        # Select mined blocks
        candidates = [
            (x, y) for x in range(self.windowWidth) for y in range(self.windowHeight)
        ]
        ...
        for x, y in candidates:
            if (
                (x < 2 and y < 2)
                or (x < 2 and y > self.windowHeight - 3)
                or (x > self.windowWidth - 3 and y < 2)
                or (x > self.windowWidth - 3 and y > self.windowHeight - 3)
            ):
                continue
            ...
            for y in range(self.windowHeight):
                if self.is_mine[y][x]:
                    for new_x, new_y in [
                        ...
                        if (0 <= new_x < self.windowWidth) and (
                            0 <= new_y < self.windowHeight
                        ):
                            self.neighboring_mine_count[new_y][new_x] = (
                                self.neighboring_mine_count[new_y][new_x] + 1
                            )
        if self.verbose > 0:
            print(
                f"Successfully created new board {self.windowHeight}x{self.windowWidth} with {self.mines} mines."
            )
                ...
                if self.pressed[y][x]:
                    ...
                    if self.neighboring_mine_count[y][x] > 0:
                        text = self.font.render(
                            str(self.neighboring_mine_count[y][x]),
                            True,
                            self.colors[self.neighboring_mine_count[y][x]][1],
                        )
                        textRect = text.get_rect()
                        textRect.center = (
                            (x + 0.5) * self.blockSize,
                            (y + 0.5) * self.blockSize,
                        )
                ...
                if self.flagged[y][x] and not self._bombed:
                    if self.flag_image is None:
                        ...
                        self.flag_image = pygame.transform.scale(
                            self.flag_image, (self.blockSize, self.blockSize)
                        )
                    surface.blit(
                        self.flag_image, (x * self.blockSize, y * self.blockSize)
                    )
                if self.is_mine[y][x] and self._bombed:
                    if self.mine_image is None:
                        ...
                        self.mine_image = pygame.transform.scale(
                            self.mine_image, (self.blockSize, self.blockSize)
                        )
                    surface.blit(
                        self.mine_image, (x * self.blockSize, y * self.blockSize)
                    )
        for x in range(self.windowWidth):
            pygame.draw.rect(
                surface,
